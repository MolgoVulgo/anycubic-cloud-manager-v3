...
Le chemin activé exige de la matière de radeau dès la couche 1, détermine la fin du radeau et attribue les phases globales `Raft`, `SupportsOnly`, `ModelAndSupports` et `ModelMostly`. Toute matière comprise entre le radeau et la première couche de pièce détectée appartient par construction aux supports. Pendant les phases mixtes, les composants candidats sont suivis dans une forêt orientée enracinée : un seul parent structurel est conservé, une branche peut se diviser, aucune fusion structurelle de branches n'est créée et les contacts secondaires ne deviennent des croisillons que lorsque leur trajectoire est approximativement diagonale à 45 degrés. Un contact terminal ne devient `Head` qu'après validation d'un rétrécissement local. Un support qui démarre sur une pièce déjà établie doit également naître d'une racine localement étroite ; une excroissance large de la pièce ne devient pas un support simplement parce que son extrémité supérieure se rétrécit.

Lorsqu'une tête rétrécie atteint un composant non compact de la pièce, l'analyseur considère le passage d'une petite section à une section plus large comme la frontière sémantique. Une branche ne peut devenir un contact qu'après ne plus posséder de continuation ni de division sur la couche courante ; la simple proximité de la pièce pendant que la branche continue à se rétrécir ne peut pas produire un contact anticipé. Le support s'arrête sur sa dernière couche libre de tête terminale. Le composant connecté complet de la couche de contact suivante appartient à la pièce, y compris les pixels qui recouvrent exactement la pointe : aucune sémantique Support n'est projetée dans la pièce et aucune couche ultérieure ne peut en hériter. Le plan d'échantillonnage obligatoire conserve malgré tout la dernière tête libre et la première couche de contact afin que le changement de couleur se produise au bon plan Z. Les contacts sans rétrécissement sont rejetés et conservent la couleur de la pièce.

Le résultat d'analyse reste compact. Chaque couche conserve sa phase et les identifiants triés et uniques des composants support libres ; les masques décodés et les bitmaps sémantiques complets ne sont pas retenus. Le champ de compatibilité des runs support projetés reste présent mais demeure vide pour les contacts avec la pièce. Chaque contact pièce validé enregistre deux échantillons source obligatoires : la dernière couche libre de la tête terminale et la première couche de contact. Pendant la construction du mesh, `materializeLayerSemantics()` réextrait uniquement une couche retenue et un `SupportMaskProvider` traduit les runs `Raft` et `Support` libres vers le bit support existant. Lorsque l'analyse Supports est active, le mesher fusionne ces échantillons obligatoires avec le stride fixe de l'aperçu rapide, puis trie et déduplique le plan. Une extrusion ne peut ainsi plus traverser une transition support/pièce ignorée, tandis que la couche de contact reste entièrement en couleur pièce. Lorsque l'analyse est désactivée, la liste obligatoire est ignorée et le stride classique reste strictement inchangé. Le résultat ne dépend ni de la taille des chunks de rendu ni du nombre de workers. Les sections dynamiques basse et haute sélectionnent le dernier échantillon sémantique retenu inférieur ou égal à la couche matière demandée et conservent donc les couleurs du mesh principal.
...
`accloud_support_analyzer` valide le radeau obligatoire, les phases supports seuls et mixtes, une forêt de supports enracinée sans fusion structurelle, les divisions de branches, les croisillons approximativement à 45 degrés, les supports démarrant sur la pièce, le rétrécissement terminal, le report du contact tant qu'une branche possède un enfant structurel, le rejet des coques creuses, les index compacts déterministes, l'annulation et la rematérialisation exacte couche par couche.
