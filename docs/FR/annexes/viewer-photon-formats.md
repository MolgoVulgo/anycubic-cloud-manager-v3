...
P6/P6.1 ajoute un accélérateur Vulkan Compute optionnel pour la boucle régulière la plus coûteuse de la recherche de lignée inverse/pièce : le comptage des recouvrements translatés. Le graphe sémantique, l'ordre des couches, les départages, la réconciliation et les commits **ne** sont pas déplacés sur le GPU. `auto` est le mode runtime normal. CMake active le backend uniquement lorsque le SDK Vulkan et un compilateur de shader SPIR-V (`glslc` ou `glslangValidator`) sont tous deux disponibles ; sinon le même chemin binaire est construit avec uniquement l'implémentation CPU canonique. Au runtime, `auto` retombe également sur le CPU si aucune file compute Vulkan exploitable ne peut être créée ou si un job GPU échoue. `cpu` force l'implémentation de référence, tandis que `vulkan` exige l'initialisation du backend et rapporte une erreur d'analyse si celui-ci ne peut pas être créé. Seuls les composants dont la surface atteint `vulkanMinimumComponentAreaPixels` (32 768 pixels par défaut) sont candidats au GPU, afin que les petits composants clairsemés restent sur le CPU et évitent le coût de transfert/dispatch. P6.1 remplace l'ancien schéma une invocation par translation par un workgroup par translation dont 64 lanes se partagent puis réduisent la plage de mots denses. Les requêtes de composants concurrentes du pool sémantique sont regroupées par un dispatcher Vulkan dédié (jusqu'à 64 jobs sous réserve des limites de storage buffers du périphérique), packées dans des buffers staging persistants, copiées vers des buffers de stockage `DEVICE_LOCAL` réutilisables, exécutées dans une seule soumission, relues en une fois puis rendues à l'ordre exact des candidats et au départage déterministe CPU. Le backend supprime ainsi le mutex Vulkan global par composant et augmente l'occupation GPU sans faire du résultat GPU une autorité sémantique. Il reste indépendant du renderer OpenGL 3.3 et conserve un fallback CPU obligatoire.
