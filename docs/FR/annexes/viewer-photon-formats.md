...
Le chemin activé exige de la matière de radeau dès la couche 1 et attribue les phases globales `Raft`, `SupportsOnly`, `ModelAndSupports` et `ModelMostly`. Le radeau est désormais défini par une hauteur physique configurée (`raftHeightMillimetres`, 0,5 mm par défaut) convertie en nombre de couches natives à partir de `pitchZMillimetres`. Toute couche située dans cette fenêtre Z reste `Raft` même si sa forme XY évolue fortement : plaque, grille, semelles, rebords ou géométrie de type cuvette ne dépendent plus d'une comparaison avec le premier masque. La première couche native immédiatement après cette hauteur est la première couche de supports et constitue la vérité racine de la passe bas vers haut. Indépendamment, chaque pixel exposé de la dernière couche native constitue la vérité racine de la passe pièce haut vers bas. La hauteur de couche PWSZ intervient donc dans la détermination physique du préfixe de radeau ; hors de ce préfixe, les relations support/pièce restent évaluées entre couches natives consécutives.

L'analyseur construit d'abord les informations de composants connexes et de relations natives, puis P2 établit un canal de **provenance support issue du radeau** distinct du verdict sémantique final. Tous les composants de la première couche native située immédiatement après le radeau sont des racines de provenance support obligatoires, sans condition de diamètre, surface ou rétrécissement. Sur les couches suivantes, cette provenance est propagée uniquement lorsqu'une relation géométrique existe dans le graphe immuable des couches natives adjacentes (recouvrement ou proximité matière dans la fenêtre de mouvement configurée). Une croissance de section, un rétrécissement, un contact candidat ou même un verdict provisoire `Model` de l'ancienne logique montante ne peut donc plus effacer cette provenance. Une matière qui apparaît plus tard sans chemin géométrique vers une racine de la première couche post-radeau ne reçoit pas cette provenance. Le résultat compact expose cette preuve séparément via `raftSupportProvenanceComponentIds`; P2 conserve encore les heuristiques historiques de contact pour le verdict sémantique courant, afin que leur remplacement par l'arbitrage topologique des phases suivantes puisse se faire sans perdre la source de vérité support. Les ratios de surface, l'historique de rétrécissement, les résidus de mouvement et la couverture de fusion deviennent des valeurs de confiance/preuve : ils peuvent signaler une frontière terminale plausible du support, mais ne possèdent plus à eux seuls la décision sémantique finale. La passe sémantique 2 réutilise les descriptions natives conservées dans l'ordre décroissant et propage indépendamment un masque clairsemé de pièce vers le bas. La jonction entre les deux passes est exacte au niveau couche : la dernière couche native traitée par la passe 1 est aussi la première couche native traitée par la passe 2, indépendamment du stride du viewer et de la parité du numéro de couche. P5 supprime cette dépendance sémantique de la provenance descendante. La dernière couche native reste la racine supérieure obligatoire et tout composant plus bas sans continuité géométrique vers la couche native suivante devient une racine topologique de maximum local en Z, indépendamment de la première couche pièce déduite par la passe 1 et indépendamment de la provenance support issue du radeau. Ces racines et leurs descendants sont déterminés uniquement par le graphe immuable des couches adjacentes et peuvent donc traverser un composant que la passe 1 considère comme support. Pour garder P5 dans son périmètre, une preuve descendante introduite uniquement par un nouveau maximum local ou par le franchissement d’une barrière de support entier est propagée vers la couche inférieure mais ne consomme pas encore une partition support protégée ; P6 prendra en charge cette nouvelle frontière de conflit. La réconciliation confronte ensuite les deux provenances sur le même composant. Une preuve support seule donne support, une preuve pièce seule donne pièce et les deux preuves simultanées sont séparées en runs mixtes au lieu de forcer tout le composant connexe dans une seule catégorie.

Les supports et la pièce ne sont pas séparés par un diamètre ou une surface absolue de composant. P3 remplace la représentation structurelle en forêt stricte par un graphe topologique orienté : un parent principal reste conservé pour l’historique de trajectoire et les calculs locaux, mais les autres parents portant la provenance support issue du radeau ne sont plus perdus. Lorsqu’un composant support déjà retenu reçoit plusieurs parents de provenance support, les contacts secondaires sont d’abord testés avec la détection de croisillon existante ; ceux qui ne sont pas des croisillons deviennent des arêtes `join` vers un nœud `junction`. Une division reste représentée par `split`. Le graphe peut donc exprimer explicitement une convergence de plusieurs branches support sans la réduire à une pseudo-fusion à parent unique. L’association des parents continue d’utiliser le recouvrement des runs raster natifs et la distance matière exacte entre runs ; un simple recouvrement de boîtes englobantes ne crée pas de parent à travers une zone vide. Les anciens paramètres `minimumSupportParentCoverageRatio` et `minimumSupportFusionCoverageRatio` sont encore exposés pour compatibilité diagnostique pendant la migration, mais ils ne décident plus si une convergence structurelle autorisée devient un `junction`. La raison diagnostique correspondante est désormais `support_junction_continuation`; les champs historiques de couverture/fusion restent temporaires jusqu’au nettoyage final. P3 ne remplace pas encore les heuristiques historiques de frontière support/pièce : il rend d’abord la topologie support multi-parent explicite afin que les passes descendantes et la future résolution de contact puissent l’exploiter sans ambiguïté.

P4 contraint maintenant la géométrie de déplacement du réseau support à des **segments rectilignes par morceaux**. Une lignée peut être verticale ou inclinée ; un changement réel de vecteur termine le segment courant et démarre un nouveau segment au nœud suivant, au lieu d’être interprété comme une courbure continue. La direction est estimée sur l’historique récent du parent principal jusqu’à la dernière racine/division/jonction/rupture de segment, et le résidu maximal autorisé est contrôlé par `supportSegmentMaximumResidualPixels`. Les croisillons/entretoises utilisent désormais cette géométrie : une lignée secondaire rectiligne suffisamment établie est classée `Brace` lorsqu’elle rejoint un support principal établi avec un angle physique proche de `braceTargetAngleDegrees` (45° par défaut), dans `braceAngleToleranceDegrees`. L’angle est calculé avec `pitchXMillimetres`, `pitchYMillimetres` et `pitchZMillimetres`, et non avec une dérive raster en pixels/couche. Une entretoise reste une relation secondaire et ne remplace jamais le parent structurel principal ; les convergences qui ne satisfont pas la géométrie d’entretoise restent des `join`/`junction`.

Dans la passe montante de preuve, un rétrécissement suivi d'une croissance locale ouvre l'état `contact candidat` ; il ne transforme pas immédiatement le composant courant en pièce et ne suffit plus à fixer la frontière sémantique finale. Une pointe terminale n'est pas établie par une ancienne chute isolée par rapport à la plus grande section de la fenêtre d'historique. La réduction relative doit aussi être confirmée par un effondrement final immédiat, par plusieurs diminutions significatives dans la lignée récente, ou par un rebond borné après un rétrécissement réel. Un plateau de support stable après une ancienne traverse ou fusion de branches ne peut donc plus devenir une pointe terminale. La décision reste locale à la branche suivie et ne dépend jamais de la présence de matière pièce ailleurs dans l'impression. Une croissance unique atteignant le seuil relatif configuré peut ouvrir le candidat. Une croissance plus faible ne peut l'ouvrir que si le centre sort aussi de l'enveloppe de déplacement normale **et** si la section parente, translatée sur le centre courant, ne conserve plus le recouvrement de forme minimal configuré. Le déplacement du centre seul ne constitue jamais une preuve de contact avec la pièce. Si la forme translatée et la trajectoire prédite de la branche restent cohérentes, le composant demeure support avec la raison diagnostique `support_motion_continuation`. Le candidat doit ensuite persister. Il est confirmé soit par une première section localement brutale par rapport à la pointe terminale, soit par une croissance cumulée depuis sa première couche au fil des couches natives suivantes. Une séquence stationnaire, décroissante, expliquée par le mouvement ou non confirmée est réintégrée à la branche support. Les jonctions et croisillons multi-support sont désormais décrits séparément par le graphe topologique ; les heuristiques historiques de contact support/pièce restent toutefois en place jusqu'aux phases de réconciliation prévues.

La confirmation montante d'un contact reste une preuve utile et enregistre toujours la paire d'échantillons tête terminale/contact, mais la frontière finale n'est engagée que lorsque la provenance pièce descendante atteint la même zone. Un composant enraciné dans le radeau qui appartient encore entièrement à la passe 1, sans projection pièce montante ni candidat de contact actif, conserve toute sa section native courante comme support : les pixels nouvellement exposés par le rétrécissement, l'élargissement local ou la translation ne deviennent pas de la pièce au seul motif que la passe descendante voit de la matière pièce une couche plus haut. Dès qu'un composant mixte est atteint, les deux preuves peuvent en revanche se recouvrir sur les mêmes pixels. Les pixels de preuve pièce descendante qui recouvrent directement le cœur support hérité sont absorbés par la sémantique pièce ; les pixels support hors de cette empreinte descendante restent support. Un recouvrement partiel produit donc une projection mixte, tandis qu'une couverture inverse complète termine cette empreinte support au lieu de conserver un îlot support figé dans le volume. Le plan d'échantillonnage obligatoire conserve la frontière terminale/pièce ainsi validée afin que le mesh change de couleur au bon plan Z.

La passe descendante possède la provenance pièce. Elle part de la dernière couche native et des racines P5 de maximum local en Z, suit le recouvrement exact ou une translation bornée qui maximise la matière pièce conservée et utilise le masque descendant immédiatement antérieur pour départager les translations à recouvrement égal. La preuve support de la passe 1 peut encore départager des translations géométriquement équivalentes afin d'éviter qu'une lignée pièce saute sur un support voisin, mais elle n'empêche plus la création ni la propagation de la lignée descendante indépendante. L'empreinte pièce descendante héritée est calculée d'abord, puis réconciliée run par run avec le cœur support courant : leur intersection devient pièce, les runs support restants demeurent support et le masque pièce transmis à la couche inférieure contient les pixels absorbés. La provenance pièce peut ainsi rétro-propager au travers d'une vraie région de volume sans consommer une matière support qui n'est pas réellement atteinte par la preuve descendante. Si tous les pixels du cœur support sont couverts, la région finale devient pièce ; si seule une partie est couverte, le composant reste sémantiquement mixte.

Le résultat d'analyse reste compact. Chaque couche conserve sa phase, les identifiants triés et uniques des composants entièrement support et des runs support projetés clairsemés uniquement pour les composants mixtes ; les masques décodés et les bitmaps sémantiques complets ne sont pas retenus dans l'index final. Chaque contact pièce validé enregistre deux échantillons source obligatoires : la dernière couche libre de la tête terminale et la première couche de contact. Pendant la construction du mesh, `materializeLayerSemantics()` réextrait uniquement une couche retenue et un `SupportMaskProvider` traduit le `Raft`, les composants entièrement `Support` et les runs support projetés vers le bit support existant. L'index final compact est produit après les deux parcours sémantiques indépendants. Chaque couche PWSZ native est désormais décodée et soumise à l'extraction des composants connexes une seule fois, pendant la fenêtre de préparation montante. Les `LayerDescription` compactes obtenues (bornes, surfaces, centres et runs natifs des composants) sont conservées pendant l'analyse des supports puis réutilisées directement par la passe sémantique descendante. P6.5 prépare toutes les descriptions natives compactes avant la réconciliation sémantique, puis répartit chaque paire de couches sémantiques adjacentes en lots workers contigus. Chaque lot construit des arêtes de preuves clairsemées immuables contenant le recouvrement, la distance matière exacte et la distance des centres ; la paire qui traverse une frontière de lot étant elle-même une arête, la continuité est explicite et cette étape ne nécessite pas de halo dupliqué. Les mutations/classifications montantes et descendantes du graphe restent strictement ordonnées entre les couches natives et dans l'ordre original des composants, ce qui préserve l'autorité sémantique déterministe existante. L'ordonnanceur persistant à priorité de P4, dimensionné par `render3d.workerCount`, reste l'unique ressource workers pour la préparation, la construction des preuves et les travaux sémantiques par composant, sans empiler de pools propres à l'analyseur. P6.3 dimensionne la fenêtre de préparation de façon adaptative depuis le nombre de workers configuré et un budget par défaut de 256 Mio pour les masques natifs ; sur une station à 16 workers, jusqu'à 16 préparations peuvent donc être en vol lorsque la résolution tient dans ce budget, tandis que les masques sont libérés dès que la comparaison du radeau n'en a plus besoin. Les sources déclarant les lectures concurrentes utilisent directement cette fenêtre bornée ; les autres sérialisent uniquement `loadMask()` tandis que l'ordonnanceur partagé continue de paralléliser le travail indépendant autour. Cette organisation supprime la surallocation de pools propre à l'analyseur et conserve des résultats sémantiques identiques à 1 et N workers. La géométrie d'une paire candidate adjacente est calculée une seule fois dans le graphe P6.5 et le tuple recouvrement/distance matière/distance centre est réutilisé ensuite par la réconciliation au lieu de recalculer la même paire. Aucun bitmap sémantique complet n'est conservé entre les couches ni entre les passes. Lorsque l'analyse Supports est active, le mesher fusionne les échantillons obligatoires avec le stride fixe de l'aperçu rapide, puis trie et déduplique le plan. Une extrusion ne peut ainsi plus traverser une transition support/pièce ignorée tout en préservant les deux sémantiques dans un composant mixte. Lorsque l'analyse est désactivée, la liste obligatoire est ignorée et le stride classique reste strictement inchangé. Le résultat sémantique ne dépend ni de la taille des chunks de rendu ni du nombre de workers. La pression d'allocation de ce chemin CPU est en plus bornée par la réutilisation des buffers de scanlines et de préparation par couche, un index dense des racines du disjoint-set avec réservation exacte des runs par composant, des nœuds de graphe qui référencent les composants immuables déjà conservés au lieu de dupliquer leurs vecteurs de runs, ainsi que la normalisation et l'intersection sur place des masques de runs clairsemés en ne parcourant que les lignes touchées. P5 ajoute une couche d’accélération CPU portable sans changer cette sémantique : l’extraction des composants connexes lit directement les lignes de mots contigus du `BinaryMask` et, sur les CPU x86 disposant d’AVX2, saute quatre mots 64 bits nuls à la fois avant de retomber sur exactement la même extraction scalaire des runs. Les lignes clairsemées très fragmentées peuvent aussi être promues vers des bitsets 64 bits bornés uniquement lorsque l’empreinte en mots du bitset ne dépasse pas un quart du nombre d’intervalles canoniques ; `countSet`, les recouvrements translatés et les intersections de masques peuvent alors utiliser des opérations par mots, avec dispatch AVX2 pour l’intersection sur les CPU x86 compatibles et implémentation scalaire partout ailleurs. Les runs clairsemés restent autoritaires, le cache bitset reste local à la ligne et transitoire, et aucun bitmap sémantique complet n’est conservé entre les couches. Les sections dynamiques basse et haute sélectionnent le dernier échantillon sémantique retenu inférieur ou égal à la couche matière demandée et conservent donc les couleurs du mesh principal.


P6/P6.1/P6.2 ajoute un accélérateur Vulkan Compute optionnel pour la boucle régulière la plus coûteuse de la recherche de lignée inverse/pièce : le comptage des recouvrements translatés. Le graphe sémantique, l'ordre des couches, les départages, la réconciliation et les commits **ne** sont pas déplacés sur le GPU. `auto` est le mode runtime hybride normal. CMake active le backend uniquement lorsque le SDK Vulkan et un compilateur de shader SPIR-V (`glslc` ou `glslangValidator`) sont tous deux disponibles ; sinon le même chemin binaire est construit avec uniquement l'implémentation CPU canonique. Au runtime, `auto` retombe également sur le CPU si aucune file compute Vulkan exploitable ne peut être créée ou si un job GPU échoue. `cpu` force l'implémentation de référence. L'ancienne préférence runtime full GPU `vulkan` a été supprimée. Seuls les composants dont la surface atteint `vulkanMinimumComponentAreaPixels` (32 768 pixels par défaut) sont candidats au GPU, afin que les petits composants clairsemés restent sur le CPU et évitent le coût de transfert/dispatch. P6.1 a introduit le regroupement des requêtes concurrentes via un dispatcher Vulkan dédié avec staging persistant et buffers `DEVICE_LOCAL` réutilisables. P6.2 remplace le workgroup long par translation par une grille 3D `tuile × translation × job`. Le fallback dense utilise des tuiles de 4096 mots ; le chemin principal de lignée sur le modèle stable évite de reconstruire un bitmap source dense et envoie les runs sémantiques canoniques du composant par tuiles de 64 runs. Chaque tuile produit un popcount partiel réduit sur le GPU dans le compteur de la translation. La référence complète du modèle stable est rasterisée une seule fois par couche sémantique et reçoit une clé de génération afin que les jobs compatibles réutilisent la même référence device-local au lieu de la renvoyer pour chaque composant. Les jobs concurrents restent regroupés jusqu'à 64 requêtes sous réserve des limites Vulkan de storage buffers et de workgroups, puis les résultats reviennent dans l'ordre exact des candidats et le départage déterministe CPU. Cela augmente le parallélisme au niveau workgroup et réduit la rasterisation hôte/le trafic PCIe sans faire du résultat GPU une autorité sémantique. Le backend reste indépendant du renderer OpenGL 3.3 et conserve un fallback CPU obligatoire.
...
La modal du viewport expose la phase active via `viewer.loadingPhase`. P6.5 conserve une progression monotone sur trois phases de travail de l'analyse des supports avant le maillage : préparation de la géométrie/des preuves natives, réconciliation sémantique montante puis réconciliation sémantique descendante. Le plan effectif de couches retenues est ensuite maillé pour l'aperçu. Les diagnostics runtime sont écrits dans `render3d.jsonl` avec le composant `support_analysis` et les événements `started`, `progress`, `phase_detected`, `completed`, `skipped`, `cancelled`, `failed` et `materialization_failed`. L'événement `started` indique `requested_workers` ainsi que `concurrent_mask_loads` selon le contrat de la source ; la fin de l'analyse indique le nombre d'échantillons sémantiques obligatoires. Les événements de début et de fin du mesher indiquent `base_sample_count`, `forced_semantic_sample_count` et `effective_sample_count`. L'événement de fin indique les runs de supports par composants entiers, les runs support projetés utilisés par les composants mixtes, `terminal_support_stops`, `expanding_model_contacts`, `maximum_model_expansion_ratio`, les pixels d'expansion rejetés ainsi que les contacts sans rétrécissement. `projected_contact_pixels` reste nul, car aucun pixel support n'est projeté dans une zone de contact pièce confirmée. Les logs contiennent uniquement des compteurs agrégés et les bornes de phases, jamais les composants ou runs individuels.
...
`--workers N` accepte `1..16`, vaut `1` par défaut pour des diagnostics reproductibles et dimensionne le pool partagé de l'analyse des supports utilisé par la préparation bornée des couches, la classification montante des composants, la recherche de lignée pièce et la réconciliation descendante ; P6.3 rend la fenêtre de préparation adaptative (nombre de workers + budget par défaut de 256 Mio pour les masques natifs) au lieu de la limiter à quatre, tandis que les commits couche/composant restent ordonnés et que la sortie sémantique doit rester identique. Le rapport JSON indique la valeur choisie dans `analysis_workers`. `--verify-materialization` relit toutes les couches natives et vérifie que l'index compact recrée exactement les totaux de runs radeau/support enregistrés. `--compute auto|cpu` sélectionne le mode compute standard de P6 et `--vulkan-min-area N` modifie uniquement le seuil de surface composant utilisé pour décider si le lot de recouvrements translatés mérite un dispatch Vulkan. Le résumé JSON indique si Vulkan Compute a été compilé/activé, le périphérique choisi, les nombres de jobs éligibles/soumis/GPU/fallback, les dispatchs réussis/échoués, la taille maximale d'un lot regroupé, les octets transférés ainsi que les temps de préparation hôte, d'attente de file et d'exécution des lots. P6.2 expose aussi les jobs en runs compacts, les uploads/réutilisations de référence résidente et le nombre de workgroups soumis ; sur un run réellement accéléré, `vulkan_submitted_workgroups` doit être très supérieur au nombre de jobs GPU puisque chaque gros composant est découpé en nombreuses tuiles. Le viewer journalise l'activation et le périphérique dès le démarrage puis répète ces compteurs dans les progressions par décile, ce qui permet de diagnostiquer une analyse longue même si elle est annulée. P6.3 ajoute dans ces mêmes progressions et dans le résumé JSON la capacité de fenêtre, les couches préparées, le maximum en vol et les temps détaillés de préparation/sémantique. P6.4 ajoute les compteurs d'appels/jobs de lots sémantiques couche ainsi que les temps de classification/commit/lignée/commit de lignée montants et de préparation/commit descendants. Le benchmark du gros fichier a montré que le lot couche à translation nulle ralentissait le mode `auto` normal car le backend crée encore une requête hôte par composant ; la stabilisation retire donc entièrement ce chemin du runtime. `auto` remet les comptages exacts/enveloppe à translation nulle sur les masques CPU clairsemés tout en conservant l'accélération Vulkan de lignée translatée, tandis que `cpu` reste sur le chemin CPU canonique. P6.5 ajoute `support_semantic_evidence_us`, `support_semantic_evidence_lots`, `support_semantic_evidence_layer_pairs` et `support_semantic_evidence_edges` pour la construction parallèle du graphe entre couches adjacentes, et borne plusieurs tableaux temporaires par les seuls candidats de la couche précédente au lieu de la taille cumulative du graphe. P3 durcit en plus la frontière entre accélération et sémantique pour les deux parcours : la préparation `prepareTranslatedOverlapEvidence()` peut utiliser le chemin CPU ou Vulkan pour produire uniquement les comptages de recouvrement translaté, puis `selectBestLineageMotion()` applique sur CPU les mêmes départages déterministes historiques sans connaître le backend compute. Le GPU ne peut donc ni choisir une lignée, ni modifier un parent, un contact ou une catégorie support/pièce. Les dumps PPM sont uniquement des diagnostics et ne constituent pas des ressources runtime.
...
`accloud_support_analyzer` valide les préfixes de radeau en plaque, grille et semelles selon une hauteur physique, y compris lorsque la forme XY change à l'intérieur du radeau ; il vérifie aussi que `pitchZMillimetres` convertit cette hauteur en nombre de couches natives et qu'une configuration physique équivalente conserve la même sémantique hors radeau. P2 vérifie en plus que tous les composants de la première couche post-radeau créent obligatoirement une provenance support, que cette provenance survit aux fortes variations de section et aux verdicts provisoires de contact, et qu'elle ne se propage pas vers un îlot apparu plus tard sans continuité géométrique avec le radeau. Il valide ensuite les phases supports seuls et mixtes, un graphe support enraciné avec parent principal, divisions `split`, convergences multi-parents explicites `join`/`junction` et croisillons natifs aux couches, l'indépendance vis-à-vis des diamètres light/normal/heavy, les petites premières sections de pièce et leur croissance progressive après une pointe rétrécie, les continuations inclinées de type Torus qui doivent rester support, les plateaux stables après un ancien rétrécissement qui ne doivent pas rouvrir de contact, les convergences support multi-parents qui restent support sans dépendre du seuil historique de couverture de fusion, le rejet des faux parents à travers les zones vides d'une boîte englobante, la décision locale limitée au parent structurel, les supports démarrant sur la pièce, la continuité malgré un recouvrement temporaire avec une pièce établie, le rejet d'un faux contact interne, la confirmation différée et le repositionnement rétroactif de la frontière pour le motif Beetle 632-637, la lignée pièce persistante après un contact confirmé, la continuité pièce tenant compte d'un mouvement borné sur plusieurs déplacements raster consécutifs, la prédiction indépendante de l'empreinte support dans un composant mixte, la séparation sémantique d'un composant raster mixte pièce/support, l'absorption d'une empreinte support entièrement reprise par une lignée pièce persistante sans supprimer un support voisin encore valide, le rejet des coques creuses, les index compacts déterministes, l'annulation et la rematérialisation exacte couche par couche. P4 ajoute les régressions sur les ruptures de direction entre segments droits, la reprise d’un nouveau segment rectiligne après la rupture, le maintien de la sémantique support pendant ces changements, ainsi qu’une entretoise réelle à 45° dont la classification change lorsque les pitches anisotropes modifient son angle physique. P5 vérifie en plus qu’un maximum local en Z détaché peut initialiser une provenance pièce descendante sous l’ancienne première couche pièce de la passe 1, qu’un maximum local reste une racine même lorsque le composant porte aussi une provenance support issue du radeau, que cette nouvelle preuve atteint les couches mixtes inférieures et qu’elle ne consomme pas la partition support protégée avant l’arbitre de contact P6.
...
images/layer_XXXXXX_regions.png   carte RGB24 invisible de sélection des régions sémantiques
regions.json                      régions sémantiques, identifiants de lignée et relations entre couches adjacentes
layers/layer_XXXXXX.json          décisions, régions sémantiques et géométrie des images chargées à la demande
```

`decisions.json` enregistre aussi `state.raft_support_provenance`, qui indique la provenance P2 indépendamment de `state.rooted_in_raft` du graphe historique. `analysis.json` et les JSON de couche publient `raft_support_provenance_components`, et le résumé cumule ce nombre par observation composant/couche. P5 ventile aussi `reverse_model_seeds` en `reverse_model_top_seeds` et `reverse_model_local_maximum_seeds` afin de distinguer les racines supérieures obligatoires des racines topologiques locales. `decisions.json` enregistre les surfaces courante et parente, leur ratio, le recouvrement brut et aligné sur les centres, la distance matière exacte, les pixels ajoutés et retirés après alignement, le mouvement prédit de la branche, le résidu de mouvement, ainsi que `support_segment_angle_degrees`, `support_segment_residual_pixels`, `support_segment_linear`, `support_segment_direction_break`, `brace_angle_degrees` et `brace_angle_matched` pour auditer la géométrie P4, tous les identifiants et recouvrements des parents support correspondants, les nombres et taux de parents conservés, les preuves de diminution de la pointe terminale, l’état avant et après la décision, la sémantique choisie, un `reason_code` stable et un champ `why` lisible. Les décisions de composants mixtes exposent également `model_lineage_pixels`, `model_lineage_shift_pixels` et `model_lineage_continued`, ainsi que `reverse_model_evidence_pixels`, `forward_model_core_pixels`, `reverse_support_core_pixels`, `direct_absorbed_support_pixels`, `overhang_absorbed_support_pixels`, `absorbed_support_pixels`, `monotonic_model_locked_pixels`, `final_support_pixels`, `final_model_pixels`, `reverse_model_lineage_continued`, `reverse_model_seed`, `reverse_model_top_seed`, `reverse_model_local_maximum_seed`, `reverse_model_conflict_deferred` et `bidirectional_conflict` ; `state.support_resolution` indique si le support est conservé, partiellement/entièrement absorbé par la preuve pièce, ou partiellement/entièrement verrouillé en pièce par la monotonie finale de lignée. `forward_model_core_pixels` compte le noyau pièce déjà stabilisé par la progression bas-vers-haut qui est conservé localement lorsque la première fusion avec un support enraciné n’est pas atteinte par la preuve pièce descendante ; ce noyau n’est jamais réinjecté dans la propagation de la passe 2. `direct_absorbed_support_pixels` compte le recouvrement exact de la passe 2, tandis que `overhang_absorbed_support_pixels` compte la reprise bornée supplémentaire, uniquement autour de matière apparue entre les deux couches natives supérieures et déjà prouvée comme pièce. `monotonic_model_locked_pixels` compte les pixels support retirés parce que la même empreinte matière continue était déjà classée pièce sur la couche native précédente : une lignée passée de support à pièce ne peut plus redevenir support. Un support légitime démarrant depuis une pièce établie crée au contraire une nouvelle lignée ; seule une branche étroite enracinée dans la pièce et validée comme support est protégée, sans réactiver l’ancienne lignée support au point de connexion. Ces champs rendent auditables les preuves indépendantes bas-vers-haut/haut-vers-bas, l’avancée locale du front pièce et leur réconciliation finale. La capture de la trace est optionnelle et n’est pas activée par le viewer normal. Chaque couche source exporte trois images visibles distinctes : masque brut, résultat sémantique et décisions. L’image des décisions affiche le véritable `node_id` sur les décisions pièce, contact, composant mixte et support non standard ; les continuations support ordinaires restent sans étiquette pour préserver la lisibilité des grilles denses. Une carte invisible de composants continue d’encoder `component_id + 1` en RGB24. Une seconde carte invisible encode en RGB24 le `selection_id` local à la couche de chaque région sémantique. Après matérialisation sémantique, chaque région connexe pièce/support/radeau reçoit un `region_id` propre ; les régions adjacentes de même sémantique sont reliées entre couches natives par leur recouvrement exact de pixels pour former un `lineage_id` diagnostique. `regions.json` et chaque JSON de couche exposent la sémantique, les limites source, la surface, le composant/nœud propriétaire, la lignée, les régions parentes ainsi que leurs recouvrements et taux de couverture. Cette lignée diagnostique ajoute uniquement de l’observabilité et ne modifie ni la classification ni le graphe de `SupportAnalyzer`.

Avec `ACCLOUD_DEBUG` et le viewer expérimental actifs, `MainWindow.qml` expose l’onglet **Analyse des supports**. L’utilisateur sélectionne une pièce `.pwsz` locale, puis `SupportAnalysisBridge` lance le probe de manière asynchrone à côté de l’exécutable desktop. L’onglet est divisé entre le viewer 3D synchronisé, une pile défilable verticalement des trois images diagnostiques et un inspecteur JSON inférieur. Des boutons donnent un accès direct aux vues brute, sémantique et identifiants, et la barre de défilement verticale reste visible. Un clic sur l’image sémantique ou l’image des identifiants lit les deux cartes invisibles : le composant brut continue de résoudre le `node_id` de décision, tandis que la carte de régions résout exactement la zone pièce/support/radeau sous le curseur. L’en-tête affiche le nœud, le `region_id`, la sémantique de région et le `lineage_id`, et le JSON de décision sélectionné embarque l’objet `selected_region` complet. Un changement de plage 3D ou de couche efface la sélection et actualise les trois images ainsi que le JSON de couche. Les gros fichiers globaux ne sont pas parsés de manière eager dans QML : le bridge charge uniquement `summary.json` et le fichier `layers/layer_XXXXXX.json` sélectionné. L’onglet et le bridge sont absents des ressources de production.
