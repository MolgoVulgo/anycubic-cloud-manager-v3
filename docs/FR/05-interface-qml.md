...
Le menu Paramètres expose aussi **Workers de génération 3D** et **Couleurs 3D** lorsque le viewer est disponible. La valeur persistée `render3d.workerCount` vaut `4` par défaut, est bornée à `1..16` et s'applique à chaque nouveau PWSZ chargé. Cette même valeur borne désormais aussi la préparation des couches de l'analyse des supports lorsque **Supports** est activé : le décodage PWSZ et l'extraction des composants connexes sont préchargés en parallèle, tandis que les propagations sémantiques montante et descendante restent strictement ordonnées par couche native. Au plus une couche préparée non consommée est conservée par worker d'analyse. Une valeur plus élevée augmente donc la pression CPU et une pression mémoire transitoire bornée pendant l'analyse comme pendant le meshing ; sa modification ne change pas un mesh déjà en cours de génération.
...
Dans la modal 3D, la plage de couches est présentée sous forme d'un double curseur vertical à droite du viewport, borné visuellement par la couche maximale et la couche `1`. Le survol d'une poignée affiche son numéro dans un tooltip. La molette sur ce contrôle ne modifie que la borne haute ; la borne basse reste déplaçable uniquement à la souris. Aucun champ numérique redondant n'est affiché. Une case compacte **Supports** dans l'angle inférieur gauche sélectionne le chemin optionnel en deux passes. Un chargement lancé lorsqu'elle est désactivée utilise le mesh classique et n'exécute aucune analyse des supports. Son activation sur une scène classique recharge le PWSZ, analyse toutes les couches natives, puis reconstruit le mesh avec l'index sémantique compact. Sa désactivation sur une scène déjà analysée masque immédiatement la couleur sémantique sans modifier la géométrie. La modal de construction affiche **Analyse des supports…** puis **Création de la vue 3D…** selon la phase active. Un chronomètre du temps écoulé est affiché sous le pourcentage de progression pour l’ensemble du chargement actif, sans remise à zéro entre l’analyse des supports et la construction du mesh. Elle expose aussi **Annuler** : l'UI demande l'arrêt sans attendre le `join` du worker, invalide immédiatement la génération de scène active et efface l'état CPU/GPU partiel ; les callbacks tardifs de la génération annulée sont ignorés. Un chargement ultérieur rejoint le worker arrêté avant de démarrer une nouvelle génération.
...
## Espace de développement pour l’analyse des supports

Lorsque `ACCLOUD_DEBUG` et le viewer expérimental sont tous deux actifs, un onglet **Analyse des supports** réservé au développement est chargé depuis `resources_debug.qrc`. Il permet de sélectionner une pièce PWSZ locale et lance le probe de diagnostic externe via `SupportAnalysisBridge` ; l’opération est asynchrone, expose sa progression et peut être annulée sans bloquer le thread GUI.

L’espace utilise deux séparateurs redimensionnables. Le panneau supérieur gauche réutilise `VolumeViewerPage`, le panneau supérieur droit affiche la coupe diagnostique correspondante et les contrôles de couche, et le panneau inférieur affiche le JSON de décision, de couche ou le résumé global. La couche choisie est synchronisée dans les deux sens entre le viewer 3D et la navigation diagnostique. Les composants raster mixtes sont sélectionnables à la granularité de la région sémantique : le bridge expose le `node_id` de décision avec un `region_id` unique, sa sémantique pièce/support/radeau et un `lineage_id` diagnostique ; l’objet de région sélectionné est également intégré au JSON de décision affiché. QML se limite à afficher et déléguer. La création du bundle, le parsing JSON et les accès filesystem restent en C++, tandis que les traces complètes volumineuses restent sur disque avec un chargement par couche à la demande.

L’onglet expose désormais le même contrat de calcul stabilisé que l’analyseur runtime : **Hybride CPU + GPU** (`auto`) est le mode par défaut et conserve l’autorité sémantique côté CPU tout en déléguant à Vulkan uniquement les calculs de lignée translatée jugés rentables ; **CPU uniquement** force le chemin canonique et désactive Vulkan pour l’analyse des supports. Le mode runtime full GPU supprimé n’est pas exposé. Le nombre de workers 3D sélectionné dans les paramètres est transmis au probe diagnostique au lieu de laisser sa valeur par défaut. Pendant l’analyse, `SupportAnalysisBridge` consomme l’événement `COMPUTE_STATUS` du probe afin d’afficher si Vulkan est réellement actif, le périphérique Vulkan choisi ou la raison du repli CPU. Une fois le bundle chargé, la même zone présente les compteurs de jobs GPU/replis CPU et la concurrence de préparation observée. Les `options` du bundle persistent `worker_count`, `compute_preference`, l’état bitset, le budget mémoire de préparation et le seuil de surface Vulkan afin que la réouverture d’un bundle restitue son contexte d’exécution.
