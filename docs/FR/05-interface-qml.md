...
Dans la modal 3D, la plage de couches est présentée sous forme d'un double curseur vertical à droite du viewport, borné visuellement par la couche maximale et la couche `1`. Le survol d'une poignée affiche son numéro dans un tooltip. La molette sur ce contrôle ne modifie que la borne haute ; la borne basse reste déplaçable uniquement à la souris. Aucun champ numérique redondant n'est affiché. L'overlay du viewport affiche dans l'ordre le nom de l'imprimante, le nom du fichier avec le nombre de couches, puis l'aide de navigation. Les actions d'en-tête sont **Réinitialiser la vue** puis **Plein écran** ; les actions de pied de page sont **Imprimer** puis **Fermer**. L'impression ferme le viewer avant de transmettre la même intention d'impression cloud que la liste Fichiers.

Une case compacte **Supports** dans l'angle inférieur gauche sélectionne le chemin optionnel en deux passes. Un chargement lancé lorsqu'elle est désactivée utilise le mesh classique à pas fixe d'une couche sur deux et n'exécute aucune analyse des supports. Son activation sur une scène classique recharge le PWSZ, analyse chaque couche native du bas vers le haut à partir de la première couche après radeau puis du haut vers le bas à partir de la dernière couche, réconcilie les indices support/pièce, puis reconstruit le mesh stride-2 avec l'index sémantique compact. Les couches de transition signalées dans `forcedSampleLayers` sont ajoutées au plan d'échantillonnage afin qu'une couche de preview ignorée ne puisse pas extruder de matière support à travers la frontière support/pièce détectée. Sa désactivation sur une scène déjà analysée masque immédiatement la couleur sémantique sans modifier la géométrie. La modal de construction bloque les interactions du viewer et affiche **Analyse des supports…** puis **Création de la vue 3D…** avec la progression de la phase active.
