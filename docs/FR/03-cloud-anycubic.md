...
Le rafraîchissement des métadonnées et celui des miniatures utilisent quatre politiques explicites. La sélection des fichiers cloud depuis l’onglet Imprimantes est strictement limitée au cache et ne lance jamais de GET de miniature. L’historique d’impression est encore plus strict : le mapping conserve la référence image brute uniquement comme métadonnée, expose une image d’affichage vide et ne consulte ni le cache de miniatures ni le réseau. L’inventaire automatique de l’onglet Fichiers compare les `fileId` cloud stables avec le dernier inventaire réussi et ne télécharge que les previews des nouveaux fichiers détectés ; le premier inventaire établit la référence sans considérer tout le compte comme nouveau. Un upload réussi, y compris celui d’une impression directe, autorise la résolution des miniatures manquantes et conserve les relances différées bornées nécessaires pendant le traitement Anycubic. Seul le bouton **Rafraîchir** de l’onglet Fichiers appelle `refreshFilesAndThumbnailsAsync()` et force une reprise complète, avec invalidation des candidats locaux et effacement du cache négatif.

Une miniature locale déjà associée au même identifiant de fichier cloud est validée avant tout candidat distant, sauf pendant le rafraîchissement forcé explicite. La clé du cache distant est calculée depuis une URL canonique limitée au schéma, au host, au port et au path : une rotation de query signée réutilise donc la même image. L’URL signée complète reste uniquement en mémoire et sert exclusivement à la requête GET réelle. Pendant un même processus applicatif, une validation locale réussie est mémorisée avec le chemin, la taille et la date de modification du fichier ; toute modification invalide cette mémorisation.

Les échecs distants utilisent la même identité source canonique, et non la query signée tournante. Les erreurs transitoires conservent un délai de nouvelle tentative borné. Les contenus classés `placeholder_too_small` ou `invalid_image` sont neutralisés pour le reste du processus et ne sont retentés que par le rafraîchissement forcé explicite de l’onglet Fichiers. Les passages autorisés après upload résolvent les previews manquantes nouvellement disponibles ou jamais tentées, sans effacer les échecs de contenu permanents. Les rafraîchissements de l’historique ou des métadonnées imprimante restent donc passifs côté réseau pour les previews.
