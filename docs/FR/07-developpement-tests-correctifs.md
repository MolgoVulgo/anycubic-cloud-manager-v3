...
Pour les phases de l’analyse des supports déjà validées côté core mais nécessitant les contrôles locaux du viewer, le helper suivant exécute les validations OpenGL/Vulkan obligatoires puis le gate Qt complet `experimental-viewer-qt` :

```bash
./tools/run_support_user_validation.sh
```

Le script configure puis reconstruit `experimental-viewer-qt`, exige la présence de `accloud_render3d_shader_compile` et `accloud_support_compute_vulkan`, exécute ces deux contrôles sans accepter `Skipped`, puis lance le gate CTest complet du preset. Il s’arrête immédiatement sur le premier échec et affiche la sortie utile de la commande fautive. Un test graphique absent ou `Skipped` est signalé comme validation incomplète au lieu d’être présenté comme un succès.
...
Après exclusion du champ `analysis_workers`, les deux sorties JSON doivent être sémantiquement identiques. L'analyseur décode/décrit chaque couche native exactement une fois, conserve les descriptions compactes pour la passe sémantique descendante, réutilise les métriques d'appariement parent de chaque composant pendant la couche courante et utilise un unique pool persistant à priorité pour la préparation des couches en faible priorité ainsi que les lots prioritaires de classification montante, recherche de lignée pièce et réconciliation descendante. Les éléments prioritaires sont réclamés dynamiquement par les workers configurés tandis que le coordinateur valide les résultats dans un ordre déterministe. P6.3 dimensionne la fenêtre de masques natifs de façon adaptative au lieu de la plafonner à quatre : sa capacité est bornée par `workerCount`, les couches restantes et `preparationMemoryBudgetBytes` (256 Mio par défaut). Les lots sémantiques prioritaires continuent de préempter la préparation de fond dans l'ordonnanceur partagé. Les structures sensibles aux allocations sont également réutilisées : l'extraction des composants connexes conserve ses buffers de lignes entre les scanlines, associe les racines du disjoint-set via un index dense au lieu d'une map ordonnée, réserve exactement le stockage des runs de chaque composant et `NodeState` référence le composant immuable déjà conservé au lieu de recopier son vecteur de runs natifs. `SparseRunMask` fusionne ses lignes sur place et ne parcourt pour les intersections que les lignes touchées par l'opérande le plus clairsemé ; les vecteurs de préparation montante/descendante conservent leur capacité entre les couches. Le test `accloud_support_analyzer` vérifie un seul `loadMask()` source par couche native pour les sources concurrentes comme sérialisées, contrôle qu'un run à 16 workers élargit la préparation au-delà de quatre couches lorsque le budget de masques natifs le permet, vérifie qu'un budget mémoire volontairement réduit resserre à nouveau la fenêtre, compare le graphe/résultat sémantique à 1, 4 et plusieurs workers, contrôle la stabilité déterministe des identifiants locaux sur un cas dense de composants déconnectés et compare le chemin hybride bitset/SIMD de P5 au chemin canonique uniquement basé sur les runs sur des lignes fortement fragmentées. Le probe de diagnostic accepte `--no-bitsets` pour désactiver uniquement le cache hybride de bitsets de lignes lors des mesures A/B ; le dispatch AVX2 des scanlines conserve toujours son fallback scalaire obligatoire et ne modifie pas le contrat sémantique. P6 accepte `--compute auto|cpu` et `--vulkan-min-area N`. `auto` est le contrat runtime hybride standard : lorsque les headers/la bibliothèque Vulkan ainsi que `glslc` ou `glslangValidator` sont disponibles à la compilation et qu'une file compute Vulkan exploitable existe au runtime, les lots de recouvrements translatés rentables de la recherche de lignée pièce sont envoyés au GPU ; sinon le chemin CPU canonique est repris automatiquement. `cpu` force le chemin de référence pour les comparaisons A/B. L'ancienne préférence full GPU `vulkan` est supprimée de l'interface runtime/probe. Le résumé JSON expose `vulkan_compute_compiled`, `vulkan_compute_active`, le périphérique choisi, les jobs éligibles/soumis/GPU/fallback, les compteurs de dispatch, la taille maximale des lots regroupés, les octets transférés et les temps hôte/file/exécution. P6.2 ajoute `vulkan_run_source_jobs`, `vulkan_resident_reference_uploads`, `vulkan_resident_reference_reuses` et `vulkan_submitted_workgroups`. P6.3 ajoute `support_preparation_window`, `support_prepared_layers`, `support_max_preparation_inflight`, `support_prepare_load_us`, `support_prepare_describe_us`, `support_forward_semantic_us` et `support_reverse_semantic_us` ; ces compteurs sont aussi écrits dans les logs de progression render3d avant la fin de l'analyse. P6.4 a ajouté le regroupement des recouvrements sémantiques à translation nulle au niveau couche ainsi que la télémétrie `vulkan_semantic_layer_batch_calls`, `vulkan_semantic_layer_batch_jobs`, `support_forward_classification_us`, `support_forward_commit_us`, `support_forward_lineage_us`, `support_forward_lineage_commit_us`, `support_reverse_prepare_us` et `support_reverse_commit_us`. Le benchmark P6.4 du gros fichier a montré que ce chemin régressait le mode `auto` normal : le backend matérialisait encore une requête hôte par composant, produisant des millions d'objets de synchronisation/attentes de file pour trop peu de travail GPU utile. La stabilisation P6.5 retire ce lot couche à translation nulle du runtime de l'analyseur dans les deux modes standard. `auto` conserve les comptages exacts/enveloppes à translation nulle sur les masques CPU clairsemés et n'utilise Vulkan que pour la lignée translatée ; `cpu` reste intégralement CPU. Le test Vulkan continue de soumettre 96 requêtes runs à translation nulle en un appel bas niveau afin de protéger cette primitive backend indépendamment du modèle de coût runtime. P6.5 construit à la place une fois un graphe de preuves entre couches adjacentes indépendant de la sémantique : toutes les descriptions natives sont préparées, les paires de couches adjacentes sont réparties en lots workers contigus, et les faits recouvrement/distance matière/distance des centres sont stockés sous forme d'arêtes clairsemées de type CSR. La classification montante consomme les mêmes preuves pour les décisions parent support et pièce précédente au lieu de recalculer la géométrie des paires. Les mutations d'état montantes/descendantes finales restent ordonnées et déterministes : il s'agit de la première étape de réconciliation graphe/lots, pas encore d'une classification spéculative indépendante par lot. La télémétrie P6.5 ajoute `support_semantic_evidence_us`, `support_semantic_evidence_lots`, `support_semantic_evidence_layer_pairs` et `support_semantic_evidence_edges`. Le test analyseur compare les graphes à un et plusieurs workers, exige plusieurs lots de preuves lorsque les workers le permettent et vérifie un résultat sémantique identique. P6.5 dimensionne aussi les tableaux de comptage d'enfants, dérive de branche et contact pièce depuis les seuls candidats de la couche précédente au lieu de `states.size()` cumulatif. Le test statique `accloud_embed_spirv` exécute réellement `EmbedSpirv.cmake` avec des chemins contenant des espaces et interdit les formes `-DINPUT="..."` / `-DOUTPUT="..."` qui injectent les guillemets dans la valeur CMake et empêchent la génération du header SPIR-V. Lorsque le backend Vulkan est compilé, le CTest conditionnel `accloud_support_compute_vulkan` compare à une référence CPU indépendante le chemin shader dense tuilé et le chemin runs compacts/référence résidente sur toutes les translations. Il lance également des jobs concurrents via le backend partagé pour vérifier des résultats identiques et un regroupement réel de plusieurs jobs, puis vérifie qu'un second job runs compatible réutilise la référence device-local au lieu de la renvoyer ; il n'est ignoré avec le code 77 que si aucun périphérique Vulkan compute ne peut être initialisé au runtime. Cette mesure reste distincte du benchmark des workers de meshing ci-dessous.

`pwsz/obj_1_quant_1.pwsz` est désormais le fichier de référence pour les performances de l'analyse longue des supports. Le helper projet `tools/benchmark_support_analysis.sh` est le benchmark standard CPU contre hybride et n'expose volontairement que les deux modes runtime conservés. Il exécute `--compute cpu` puis `--compute auto`, stocke temps/télémétrie/JSON sous `benchmarks/support-analysis/<horodatage>/`, calcule un hash sémantique qui exclut les champs workers/backend/temps, échoue si CPU et hybride divergent et échoue aussi si l'ancien lot sémantique Vulkan couche est réactivé dans le runtime. Par défaut, il exige que le run hybride active réellement Vulkan ; `REQUIRE_VULKAN=0` ne doit être utilisé que pour mesurer volontairement un fallback. Depuis la racine du dépôt :

```bash
./tools/benchmark_support_analysis.sh obj_1_quant_1.pwsz
```

`WORKERS=16` et `RUNS=1` sont les valeurs par défaut. Plusieurs fichiers peuvent être nommés explicitement ; sans argument, tous les `*.pwsz` directement présents dans `pwsz/` sont testés. `BUILD_DIR=...` sélectionne un autre build du probe et `VULKAN_MIN_AREA=N` permet éventuellement de surcharger le seuil hybride ; le laisser non défini mesure la politique runtime normale. Le run hybride doit indiquer `vulkan_compute_active=true` avant d'interpréter sa durée comme une mesure CPU/GPU. `vulkan_semantic_layer_batch_calls` et `vulkan_semantic_layer_batch_jobs` doivent rester à zéro dans les deux modes standards. Un fallback CPU reste fonctionnellement valide pour `auto`, mais avec `REQUIRE_VULKAN=1` par défaut le script le classe volontairement comme mesure hybride invalide. Comparer également un build Release : `experimental-viewer-qt` hérite du type Debug et le décodage PW0, l'extraction des composants et l'orchestration du graphe côté CPU peuvent sinon sous-alimenter le GPU :
