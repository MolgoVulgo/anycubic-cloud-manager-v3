...
Le run Vulkan doit indiquer `vulkan_compute_active=true`, `vulkan_gpu_jobs>0` et `vulkan_max_batch_jobs>1` avant d'interpréter la durée comme un benchmark GPU. Un run qui retombe sur le CPU reste fonctionnellement valide en mode `auto`, mais ne constitue pas une mesure de performance Vulkan.
