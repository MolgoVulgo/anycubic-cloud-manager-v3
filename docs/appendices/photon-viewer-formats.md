...
`accloud_support_analysis_diagnostics` validates the complete bundle schema, one PNG and one lazy JSON file per layer, surface comparisons, selected semantics, stable reason codes and human-readable decision explanations.
...
images/layer_XXXXXX.png       raw mask, semantic result and decision overlay
layers/layer_XXXXXX.json      lazy per-layer decisions for the UI
```

`decisions.json` records the current and parent surfaces, their ratio, raw and centre-aligned overlap, exact material distance, added and removed pixels after alignment, predicted branch motion, motion residual, all matched support-parent node identifiers and overlaps, preserved-parent counts and coverage, terminal-taper decrease evidence, state before and after the decision, the selected semantic, a stable `reason_code` and a human-readable `why` field. Decision tracing is opt-in and is not enabled by the normal viewer path. The exported image set contains one diagnostic image for every source layer.

With `ACCLOUD_DEBUG` and the experimental viewer enabled, `MainWindow.qml` exposes the **Support analysis** tab. The user selects a local `.pwsz` part, then `SupportAnalysisBridge` launches the probe asynchronously beside the desktop executable. The tab is divided into a synchronized 3D viewer, the diagnostic image for the selected layer and a lower JSON inspector. Changing the viewer range or the layer selector updates the image and per-layer JSON. Large global files are not parsed eagerly by QML: the bridge loads `summary.json` and the selected `layers/layer_XXXXXX.json` only. The tab and bridge are absent from production resources.
