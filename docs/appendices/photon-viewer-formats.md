...
Supports enabled
-> semantic pass 1 (bottom-up): decode every native PWSZ layer and propagate
   support provenance from the first layer after the raft
-> semantic pass 2 (top-down): reread native layers from the last layer and
   propagate independent model provenance downward
