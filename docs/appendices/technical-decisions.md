...
| D-021 | The desktop viewer backend uses `QQuickFramebufferObject` with Qt OpenGL and exact shader Z clipping. | It uses public Qt 6 APIs, matches the existing `render3d/gl` boundary, and is enabled by desktop presets including production; the historical build gate is retained for profile isolation, not as a product-maturity marker. |
