...
The Settings menu also exposes **3D generation workers** when the viewer is available. The persisted `render3d.workerCount` value defaults to `4`, is clamped to `1..16`, and is applied to every newly loaded PWSZ. Increasing it raises CPU and memory pressure; changing it does not alter a mesh already being generated.
