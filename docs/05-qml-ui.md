...
In the 3D modal, the layer range is exposed as a vertical dual-handle slider on the right side of the viewport, visibly bounded by the maximum layer and layer `1`. Hovering a handle shows its layer number in a tooltip. Wheel input over the control changes only the upper bound; the lower bound remains mouse-drag only. No redundant numeric bound fields are displayed. The viewport overlay orders machine name, file name plus layer count, then the navigation hint. Header actions are **Reset view** then **Full screen**; footer actions are **Print** then **Close**. Printing closes the viewer before forwarding the same cloud-file print intent used by the Files list.

A compact **Supports** checkbox in the lower-left corner selects the optional two-pass path. A load started while it is disabled uses the classic fixed stride-two mesh path and performs no support analysis. Enabling it on a classic scene reloads the PWSZ, analyzes every native layer bottom-up from the first post-raft layer and top-down from the final layer, reconciles support/model evidence, then rebuilds the stride-two mesh with the compact semantic index. Transition layers reported in `forcedSampleLayers` are added to the sampling plan so a skipped preview layer cannot extrude support material across the detected support/model boundary. Disabling support colouring on an already analyzed scene immediately hides the semantic colour without changing geometry. The build modal blocks viewer input and displays **Analyzing supports…** and **Creating 3D view…** with progress according to the active phase.
