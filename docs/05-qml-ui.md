...
The workspace uses two resizable split views. The upper left pane reuses `VolumeViewerPage`; the upper right pane shows the corresponding diagnostic slice and layer controls; the lower pane displays the decision, layer or global-summary JSON. The selected layer is synchronized in both directions between the 3D viewer and diagnostic navigator. QML only displays and delegates. Bundle creation, JSON parsing and filesystem access remain in C++, and large complete traces are kept on disk with per-layer lazy loading.
