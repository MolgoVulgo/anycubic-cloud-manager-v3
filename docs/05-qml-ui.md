...
The Settings menu also exposes **3D generation workers** and **3D colors** when the viewer is available. The persisted `render3d.workerCount` value defaults to `4`, is clamped to `1..16`, and is applied to every newly loaded PWSZ. The same value now also bounds support-analysis layer preparation when **Supports** is enabled: PWSZ decoding and connected-component extraction are prefetched concurrently, while the bottom-up and top-down semantic propagation remains strictly ordered by native layer. At most one unconsumed prepared layer per analysis worker is retained. Increasing the value therefore raises CPU and bounded transient memory pressure in both analysis and meshing; changing it does not alter a mesh already being generated.

The persisted `render3d.palettePreset` value selects one of five part/support/background palettes: `technical_cyan`, `industrial_amber`, `mineral_ivory`, `night_coral` or `light_graphite`. Invalid persisted values are normalized to `technical_cyan`. Applying a preset updates all three colors together, propagates to an already open viewer, and schedules only a repaint through `meshColor`, `supportColor` and `backgroundColor`; it does not reload the PWSZ or rebuild the mesh. Free-form color entry is not exposed.

In the 3D modal, the layer range is exposed as a vertical dual-handle slider on the right side of the viewport, visibly bounded by the maximum layer and layer `1`. Hovering a handle shows its layer number in a tooltip. Wheel input over the control changes only the upper bound; the lower bound remains mouse-drag only. No redundant numeric bound fields are displayed. A compact **Supports** checkbox in the lower-left corner selects the optional two-pass path. A load started while it is disabled uses the classic mesh path and performs no support analysis. Enabling it on a classic scene reloads the PWSZ, analyzes every native layer, then rebuilds the mesh with the compact semantic index. Disabling it on an already analyzed scene immediately hides the semantic colour without changing geometry. The build modal displays **Analyzing supports…** and **Creating 3D view…** according to the active phase. A live elapsed timer is displayed below the progress percentage for the complete active load, without resetting between support analysis and mesh construction. It also exposes **Cancel**: the UI requests cancellation without waiting for the worker to join, immediately invalidates the active scene generation and clears partial CPU/GPU state; late callbacks from the cancelled generation are ignored. A later load joins the stopped worker before starting a new generation.
...
## Development support-analysis workspace

When both `ACCLOUD_DEBUG` and the experimental viewer are enabled, a development-only **Support analysis** tab is loaded from `resources_debug.qrc`. It lets the operator select a local PWSZ part and starts the external diagnostic probe through `SupportAnalysisBridge`; the operation is asynchronous, reports progress and can be cancelled without blocking the GUI thread.

The workspace uses two resizable split views. The upper left pane reuses `VolumeViewerPage`; the upper right pane shows the corresponding diagnostic slice and layer controls; the lower pane displays the decision, layer or global-summary JSON. The selected layer is synchronized in both directions between the 3D viewer and diagnostic navigator. Mixed raster components are selectable at semantic-region granularity: the bridge exposes the decision `node_id` together with a unique `region_id`, its model/support/raft semantic and a diagnostic `lineage_id`; the selected region object is also embedded into the displayed decision JSON. QML only displays and delegates. Bundle creation, JSON parsing and filesystem access remain in C++, and large complete traces are kept on disk with per-layer lazy loading.

The tab now exposes the same stabilized compute contract as the runtime analyzer: **Hybrid CPU + GPU** (`auto`) is the default and keeps CPU semantics authoritative while Vulkan accelerates only eligible translated-lineage work; **CPU only** forces the canonical path and disables Vulkan for support analysis. The removed full-GPU runtime mode is not exposed. The selected 3D worker setting is forwarded to the diagnostic probe instead of silently using the probe default. During a run, `SupportAnalysisBridge` consumes the probe `COMPUTE_STATUS` event so the tab shows whether Vulkan is really active, the selected Vulkan device, or the CPU fallback reason. Once the bundle is loaded, the same row exposes GPU job/fallback counts and the observed preparation concurrency. Bundle `options` persist `worker_count`, `compute_preference`, bitset state, preparation-memory budget and the Vulkan minimum-area threshold so reopening a bundle restores the execution context.
