...
| `src/accloud/render3d/` | functional standard PWSZ 3D viewer, enabled in desktop runtimes including production |
...
Production resources are declared in `src/accloud/app/resources.qrc`. Debug pages are declared separately in `resources_debug.qrc` and compiled only in debug-enabled builds. `VolumeViewerPage.qml` and `VolumeViewerDialog.qml` are packaged with the normal resource bundle. Desktop presets register `Accloud.Render3D` and expose a 3D action on every PWSZ row. Production now sets `ACCLOUD_ENABLE_EXPERIMENTAL_VIEWER=ON`, so the PWSZ action is enabled in the Release runtime; `protected-core` remains viewer-free.
...
| `prod` | Release + Qt | production runtime; debug tooling disabled, viewer enabled |
...
## Viewer isolation boundary

Photon/PWMB parsing, viewer jobs/cache and `render3d` remain outside `accloud_infra`. Desktop presets link the isolated viewer target to `accloud_cli` and expose the per-file PWSZ action, including `prod`; `protected-core` continues to exclude it.

The viewer is a standard desktop/production feature. The identifiers `ACCLOUD_ENABLE_EXPERIMENTAL_VIEWER`, `accloud_experimental_viewer`, `experimental-viewer-core` and `experimental-viewer-qt` are retained as historical internal build/test names for compatibility; they no longer describe the product maturity of the viewer. The option still defaults to `OFF` for direct CMake configurations, while `default` and `prod` explicitly enable it; `dev-debug` and `local-full` inherit the development setting, and `protected-core` explicitly disables it because that gate excludes Qt/OpenGL. Enabled desktop builds link the core to `accloud_cli`, register `Accloud.Render3D/VolumeViewer`, force the Qt Quick scene graph to OpenGL and show a 3D button on every PWSZ file row. The action downloads the file to a temporary path and opens a viewer dialog. The desktop path performs PWSZ reading and meshing outside the GUI thread, feeds a bounded `UploadQueue`, uploads layer chunks to OpenGL buffers and clips the rendered mesh to the exact inclusive layer range selected in QML. Mesh chunks are processed by four workers by default. The persisted user setting `render3d.workerCount` accepts values from 1 through 16; PWSZ mask reads are concurrent because each archive entry is reopened independently, while sources that do not advertise concurrent reads are serialized by the mesher. The default preview samples one source layer out of two while preserving the exact first/last selected layers and original Z extent; the UI can rebuild at full per-layer fidelity. Structured generation diagnostics are written with source `render3d` to the dedicated `render3d.jsonl` sink, including requested/effective worker counts and per-worker statistics. Production uses this standard viewer path. Semantic support colouring remains experimental on a separate development branch and is absent from this production snapshot; performance and robustness gates continue to validate the viewer implementation without changing its product status.

See [the viewer appendix](appendices/photon-viewer-formats.md). Viewer code must not become an implicit dependency of cloud, MQTT or production UI fixes.
