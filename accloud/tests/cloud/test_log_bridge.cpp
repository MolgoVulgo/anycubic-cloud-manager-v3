...
bool test_log_bridge_reads_jsonl_files_from_accloud_log_dir() {
    ...
    if (!expect(appOk && mqttOk && cloudOk, "Cannot create fixture log files")) {
    ...
    return expect(okFlag, "Log snapshot should be ok")
        && expect(totalEntries == 3, "Log snapshot should include all fixture lines")
        && expect(entries.size() == 3, "Entries list should include all fixture lines")
...
int main() {
    const bool ok = test_log_bridge_reads_jsonl_files_from_accloud_log_dir();
    if (!ok) {
        return 1;
    }
